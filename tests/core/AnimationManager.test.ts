...
describe('AnimationManager', () => {
    ...
    it('should warn when registering an animation with a name that already exists', () => {
        manager.register(MockAnimation as unknown as AnimateClass);
        manager.register(MockAnimation as unknown as AnimateClass);
        expect(console.warn).toHaveBeenCalledWith(`AnimationManager: An animation with the name "${MockAnimation.animationName}" is already registered. It will be overwritten.`);
    });

    it('should create an animation instance and add it to the active list', () => {
        manager.register(MockAnimation as unknown as AnimateClass);
        const instance = manager.create(MockAnimation.animationName, baseObject, sprites) as MockAnimation;
    ...
    it('should return undefined and log an error for an unregistered animation', () => {
        ...
        expect(console.error).toHaveBeenCalledWith('AnimationManager: No animation registered with the name "NonExistent".');
    });
    ...
    it('should apply the speed multiplier during update', () => {
        ...
        manager.update(0.010);
        expect(instance.update).toHaveBeenCalledWith(0.010 * 2.5);
    });
