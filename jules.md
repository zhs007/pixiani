...
为了提高 AI 生成代码的质量和可靠性，编辑器后端现在实现了一个完整的测试驱动开发（TDD）循环。这个过程对最终用户是透明的，但在内部，Agent 现在遵循以下步骤：

1.  **分析需求**: Agent 接收用户输入的动画需求。
2.  **探索代码**: Agent 调用 `get_allowed_files` 和 `read_file` 工具来学习现有动画和测试的实现方式，以确保代码风格和命名约定的一致性。
3.  **编写动画代码**: Agent 使用 `create_animation_file` 工具生成新的动画类文件。
4.  **编写测试代码**: Agent 立即为新动画编写一个 Vitest 测试文件，并使用 `create_test_file` 工具保存它。
5.  **测试与调试**:
    *   Agent 调用 `run_tests` 工具来执行它刚刚编写的测试。
    *   所有文件操作和测试都在一个与用户会话绑定的沙箱环境 (`.sessions/<session-id>`) 中进行，以避免污染项目源代码。
    *   如果测试失败，Agent 会收到详细的错误报告。
    *   Agent 使用 `update_animation_file` 和 `update_test_file` 工具来修改代码，然后重新运行测试。
    *   这个“测试-调试”循环会一直持续，直到所有测试都通过为止。
6.  **完成**: 只有当测试完全通过后，任务才被视为完成，并通知用户动画已成功创建。
